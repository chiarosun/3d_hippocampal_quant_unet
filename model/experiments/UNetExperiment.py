...
class UNetExperiment:
    ...
    def __init__(self, config, split, dataset):
        ...
        # Create data loaders
        # we are using a 2D version of UNet here, which means that it will expect
        ...
        self.model = UNet(num_classes=3) # Negative, Anterior, Posterior 
        ...
        # vs torch.nn.functional.cross_entropy (have to pass the weights)
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            self.optimizer.zero_grad() # pytorch accumulates gradients, zero out

            # Data is in batch variable. Put the slices as 4D Torch Tensors of 
            # shape [BATCH_SIZE, 1, PATCH_SIZE, PATCH_SIZE] into variables data and target. 
            # Feed data to the model and feed target to the loss function

            data = batch['image'].to(self.device, dtype=torch.float) # get batch
            target = batch['seg'].to(self.device)
            prediction = self.model(data) # pass batch, return prediction tensor
            ...
            loss = self.loss_function(prediction, target[:, 0, :, :]) # calculate loss tensor

            # What does each dimension of variable prediction represent?
            # (8, 3, 64, 64)
            # Represents (Batch Size (8), Class Probabilities (3), 2D Slice Dimensions/Patch Sizes (64 x 64) )
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for i, batch in enumerate(self.val_loader):
                ...
                # validation code to compute loss on a validation sample
        ...
        self.scheduler.step(np.mean(loss_list)) #optimize learning rate
    ...
    def run_test(self):
        ...
        for i, x in enumerate(self.test_data):
            ...
            # Compute Dice3D and Jaccard3D functions. 
