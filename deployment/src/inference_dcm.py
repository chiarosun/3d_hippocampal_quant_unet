...
def get_predicted_volumes(pred):
    ...
    volume_ant = np.sum((pred==1).astype(int))
    volume_post = np.sum((pred==2).astype(int))
...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    # clinicians. Only part efforts that will be visible to the world. 
    # Usefulness of your computations will largely
    # depend on how presented.
    ...
    draw.multiline_text((10, 90),
                        ...
                        Photometric Interpretation: {header.PhotometricInterpretation}\n \
                        Images In Acquisition: {header.ImagesInAcquisition}\n \
                        Study Date: {header.StudyDate}\n \
                        Study Description: {header.StudyDescription}\n \
                        Series Description: {header.SeriesDescription}\n \
                        Hippocampal Volume (Total): {pred_volumes['total']}\n \
                        Hippocampal Volume (Anterior): {pred_volumes['anterior']}\n \
                        Hippocampal Volume (Posterior): {pred_volumes['posterior']}\n \
                        Referring Physician Name: {header.ReferringPhysicianName}\n ",
                        (255, 255, 255), font=main_font)



    # In addition to text data in the snippet above, show images
    # Overlay mask on top of original data
    # convert a numpy array into a PIL image and draw it inside our pimg object:
    #
    ...
    # Paste the PIL image into our main report image object (pimg)
    pimg.paste(pil_i, box=(10, 410))
    pred_slice = pred_vol[slice_nums[0], :, :]
    pred_nd_img = np.flip((pred_slice/np.max(pred_vol))*0xff).T.astype(np.uint8)
    pil_i1 = Image.fromarray(pred_nd_img, mode="L").convert("RGBA").resize((500,500))
    pimg.paste(pil_i1, box=(500, 410))
...
def get_series_for_inference(path):
    ...
    # Here we are assuming that path is a directory that contains a full study as a collection of files
    ...
    # dicoms = [pydicom.dcmread(os.path.join(chosen_path, f)) for f in os.listdir(chosen_path)]
    
    # Create a series_for_inference variable that will contain a list of only 
    # those PyDicom objects that represent files that belong to the series to 
    # run inference on.
    ...
    # your algorithm. Here, we have pre-labeled output with HippoCrop tool a consistent way. 

    series_for_inference = []
    for d in dicoms:
        # look at header
        #if (d.BodyPartExamined == 'BRAIN') and (d.Modality == 'MR'):
...
if __name__ == "__main__":
    ...
    #study_dir = [dir for dir, subdirs, files in os.walk(path) if 'HCropVolume' in dir]
    # study_dir = [dir for dir in subdirs]
    # print(type(study_dir), len(study_dir), study_dir)
    study_dir = [dir for dir in subdirs if 'HCropVolume' in dir][0]
    ...
    inference_agent = UNetInferenceAgent(
        device="cpu",
        parameter_file_path=r"test_results/model.pth")
    ...
    # TODO: clean
    report_save_path = r"reports.dcm"
